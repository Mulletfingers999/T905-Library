         ...
         bootbox.prompt({
           ...
           callback: function(name){
             ...
             if (name != null && name != "") {
               ...
               var firebase = new Firebase('https://t905-libraries.firebaseio.com/books/').child(result.text);
               var today = new Date();
               ...
               firebase.set({holder: name, checked_out: today.toString(), due: due_by.toString()}, function (err) {
                 if (err) {
                   ...
                   var history_fb = new Firebase('https://t905-libraries.firebaseio.com/history').child(result.text).child(name);
                   history_fb.set({checked_out: today.toString()});
                   cordova.plugins.notification.local.schedule({
                     id: 1,
                     title: 'Merit Badge Book Coming Due',
                     text: 'The ' + result.text + ' T905 book is due by ' + pd(due_by),
                     every: 'day',
